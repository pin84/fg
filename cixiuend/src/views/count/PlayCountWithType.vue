<template>
  <div id="people-manage">
    ...
    <div v-if="typeArr.length" class="table-box">
      <CountTable v-if="!curTopBtnIndex" :numArr="numArr" :typeArr="typeArr" @toDetail="toDetail" :topList=" ['代表性作品', '作品总数']" />
      <DetailTable v-else :list="filterList" :listType="2" :topList="['剧目名称', '作者', '制作时间']" />
    </div>
...
export default {
  ...
  methods: {
    ...
    async toDetail(item, idx) {
      this.curMonth = item
      let d = { index: this.pageIndex, month: JSON.stringify(this.curType), d_type: this.curMonth  }
      let { code, data } = await this.$get(this.$api.getplaybytype, d)
      console.log(data);
    ...
    async initData() {
      ...
      typeArr.forEach(date => {
        let len = this.allData.filter(item => item.d_type == date).length
        numArr.push(len)
      })
