<template>
  <div id="people-manage">
    ...
    <div v-if="typeArr.length" class="table-box">
      <CountTable v-if="!curTopBtnIndex" :numArr="numArr" :typeArr="typeArr" @toDetail="toDetail" />
...
export default {
  ...
  methods: {
    initPage(){
      console.log(inputList[0].dropdownList);
      let typeList = inputList[0].dropdownList
      typeList.unshift('全部')
      this.type = typeList
    },
    async toDetail(item, idx) {
      this.curMonth = item
      let { code, data } = await this.$get(this.$api.getuserbymonth, { index: this.pageIndex, month: this.curMonth })
    ...
    async initData() {
      let d = {
        classify_name: this.curType == '全部' ? '' :this.curType ,
        size: 999,
        index: 1,
      }
      let { code, data } = await this.$get(this.$api.getuser, d)
      let typeArr = []
