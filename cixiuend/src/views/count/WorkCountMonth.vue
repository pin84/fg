<template>
  <div id="people-manage">
    ...
    <div v-if="typeArr.length" class="table-box">
      ...
      <DetailTable v-else :list="filterList" :topList="detailTopList" :listType="3" />
    </div>
...
import { works } from '@/config/configData.js'
export default {
  data() {
    return {
      detailTopList:['工作名称','工作分类', '工作内容'],
      topList:['统计月份', '工作总数'],
      topBtnList: [
        { id: 0, text: '数据汇总' },
        { id: 1, text: '数据明细' },
  ...
  methods: {
    initPage(){
      console.log(works[0].dropdownList);
      let typeList = works[1].dropdownList
      typeList.unshift('全部')
      this.type = typeList
    },
    ...
    async initData() {
      ...
      let { code, data } = await this.$get(this.$api.getwork, d)
      let typeArr = []
