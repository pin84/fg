  ...
  <div id="people-manage">
    <div class="search-box">
      ...
      <li class="item">
        ...
        <input class="input" v-model="addr" placeholder="请输入内容" />
      </li>
    ...
    <div v-if="tableData.length" class="table-box">
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column class="" fixed="right" label="操作" width="270">
          <template slot-scope="scope" v-if="isShowOprator">
            <el-button
              v-show="btnItem.isShow"
              class="op-btn"
              :type="btnItem.type || 'success'"
              size="mini"
              @click="showPop(scope.row, idx, btnItem)"
              v-for="(btnItem, idx) in btnList"
              :key="idx"
...
<script>
import { level, inputList } from "@/config/configData.js";
...
export default {
  ...
  methods: {
    ...
    async showPop(obj, index, btnItem) {
      ...
      console.log("-cur info list---", list);
    ...
    async comPopOptions(obj, index, type = "show") {
      ...
      switch (index) {
        ...
        case 5:
          popType = 10;
          let cList = JSON.parse(JSON.stringify(inputList));
    ...
    async search() {
      let obj = {
        successor_level: this.successor_level,
      };
      if (this.name) {
      ...
      let res = await this.$get(this.$api.getuser, obj);
