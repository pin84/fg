...
import { msgbox, loading } from "@/utils/utils.js";
export default {
  name: "Home",
  data() {
    return {
  ...
  methods: {
    async test() {
      ...
      let res = await this.$get(this.$api.test, { token });
    ...
    async login() {
      ...
      let {code,data} = await this.$post(this.$api.hsLogin, d);
