...
class Http {
    ...
    send(config) {
        return new Promise(resolve => {
                ...
                complete: (response) => {
                    if (200 !== response.statusCode && response.errMsg !== "request:fail abort") {
                        wx.showModal && wx.showModal({
                            ...
                            content: response.data ? response.data.error_msg || response.data.mesg || response.data.msg || response.data.errmsg || response.data.data || '系统错误，请稍后重试！' : response.errMsg || '系统错误，请稍后重试！',
                            confirmText: "确定",
                            showCancel: !1
                        });
                    };
            ...
            config.cancelFunc && config.cancelFunc(instance);
        });
    };
