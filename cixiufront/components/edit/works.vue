	...
	<view class="formBox">
		...
		>
			<view class="titleLine flex f_a_center f_j_between">
				<view class="titleName">工作信息</view>
			</view>
			<u-form-item label="工作名称" prop="name" borderBottom>
				<view class="formItem">
					<u--input v-model="editForm.name" type="text" inputAlign="right" border="none" placeholder="请输入工作名称"></u--input>
				</view>
			</u-form-item>
			<u-form-item label="工作分类" prop="d_type" borderBottom @click="actionType = 'workType'">
				<view class="formItem">{{editForm.d_type || "请选择工作分类"}}</view>
				<u-icon slot="right" name="arrow-right"></u-icon>
			</u-form-item>
			<u-form-item label="工作内容" prop="content"></u-form-item>
			<view class="textareaBox">
				<textarea v-model="editForm.content" class="formTextarea" auto-height placeholder="请输入工作内容"/>
			</view>
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				this.editForm = {
					name: "", // 工作名称
					d_type: this.workTypeList[0].name, // 工作类型
					content: "", // 工作内容
				}
			},
			...
			// 新增用户
			apiAddItem(data) {
				this.$apis.works.addWork(data).then(res => {
					uni.showToast({
						title: '新增成功',
						duration: 2000
					})
			...
			// 修改用户
			apiUpdateItem(data) {
				this.$apis.works.updateWork(data).then(res => {
					uni.hideLoading();
					uni.showToast({
						title: '修改成功',
						duration: 2000
					...
					setTimeout(() => {
						this.initData();
						this.$emit("callback", { model: "works", type: "save" });
					}, 2000);
				}).catch(() => {
					uni.hideLoading();
				});
