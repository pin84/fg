<template>
	<view class="container">
		...
		<view class="userBox">
			<image v-if="newsInfo._avatar" :src="newsInfo._avatar" class="infoBg" mode="aspectFill"></image>
			<view class="infoBox flex f_j_end">
				<view class="msgBox flex f_c f_j_center f_a_end">
					<view class="infoName">{{newsInfo.name}}</view>
					<view class="infoDesc"></view>
				</view>
				<!-- <image v-if="newsInfo._avatar" :src="newsInfo._avatar" class="infoAvatar" mode="aspectFill"></image> -->
				<view class="infoAvatar flex f_j_center f_a_center" v-if="newsInfo._avatar">
					<u--image :showLoading="true" :src="newsInfo._avatar" width="160rpx" height="160rpx" mode="aspectFill" radius="50%"></u--image>
				</view>
			</view>
		</view>
		<view class="messageBox">
			...
			<view class="baseBox">
				<view class="msgLine flex">
					<view class="msgLeft">作品分类</view>
					<view class="msgRight">{{newsInfo.d_type || "暂无"}}</view>
				</view>
				<view class="msgLine flex">
					<view class="msgLeft">其他名称</view>
					<view class="msgRight">{{newsInfo.orther_name || "暂无"}}</view>
				</view>
				<view class="msgLine flex">
					<view class="msgLeft">作者</view>
					<view class="msgRight">{{newsInfo.playwright || "暂无"}}</view>
				</view>
				<view class="msgLine flex">
					<view class="msgLeft">制作时间</view>
					<view class="msgRight">{{newsInfo.fistshow || "暂无"}}</view>
				</view>
				<view class="msgLine flex">
					<view class="msgLeft">地区</view>
					<view class="msgRight">{{newsInfo.showaddr || "暂无"}}</view>
				</view>
				<view class="msgLine flex">
					<view class="msgLeft">内容描述</view>
					<view class="msgRight">{{newsInfo.desc || "暂无"}}</view>
				</view>
			</view>
			
			<block v-if="videoList.length">
				<view class="tabsTitle">作品视频</view>
				<view class="videoBox">
			...
			<!-- <block v-if="audioList.length">
				<view class="tabsTitle">作品录音</view>
				<view class="audioBox">
					<u-cell-group>
			...
			<block v-if="fileList.length">
				<view class="tabsTitle">作品文件</view>
				<view class="audioBox">
					<u-cell-group>
						<u-cell v-for="(item, index) in fileList" :key="index" :title="item.name" @click="handleFile(item)">
							<u-icon name="attach" color="#ec519a" size="18" slot="icon"></u-icon>
						</u-cell>
					</u-cell-group>
				</view>
			</block>
			...
			<block v-if="imgList.length">
				<view class="tabsTitle">作品照片</view>
				<view class="imgBox">
		...
		<view class="editBox">
			...
			<view class="btnBox btnBox_fixed" v-if="hasUser">
				...
				<view class="btnItem btnItem_circle" @click="apiDelPlay">删除</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			handleEdit() {
				uni.navigateTo({
					url: `/pages/datas/edit?id=${this.pageOptions.id}&type=news`
				});
			},
			tabsClick(e) {
			...
			apiGetPlayById() {
				this.$apis.news.getPlayById({
					...
					// 处理矢量列表
					this.fileList = this.handleUrls(tempInfo?.vectorfile, tempInfo?.vectorfile_url);
					// 处理图片列表
					this.imgList = this.handleUrls(tempInfo?.img_name, tempInfo?.img_url);
					// 用户信息
					this.newsInfo = tempInfo;
				});
			},
			// 删除
			apiDelPlay() {
