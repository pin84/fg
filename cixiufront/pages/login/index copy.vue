	...
	<view class="container">
		...
		<view class="formBox">
			<view class="formLine flex f_a_center f_j_between">
				...
				<input type="text" v-model="editForm.username" class="formInput" placeholder="请输入用户名" placeholder-class="formPlaceholder" />
			</view>
			<view class="formLine flex f_a_center f_j_between">
				<text class="formName">密码</text>
				<input type="text" v-model="editForm.password" :password="true" class="formInput" placeholder="请输入密码" placeholder-class="formPlaceholder" />
			</view>
			<view class="formBtn flex f_a_center f_j_center" @click="handleSubmit">登录</view>
...
<script>
	export default {
		...
		methods: {
			handleSubmit() {
				if (!this.editForm.username) return uni.showToast({ title: '请输入用户名', duration: 2000, icon: "none" });
				if (!this.editForm.password) return uni.showToast({ title: '请输入密码', duration: 2000, icon: "none" });
				this.$apis.common.userLogin({
